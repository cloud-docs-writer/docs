...
Создание кластера БД может занять несколько минут. Проверить доступность созданного кластера вы можете перейдя из **Дашборд каталога**, выбрать **Managed Service for MySQL**. На вкладке **Кластеры**, статус кластера должен быть **Running**, состояние **Alive**. 
...
1. Создание базы данных
   ...
   1. В поле **Сервер** укажите полное доменное имя созданной вами [базы данных.](../_assets/bitrix-website/mysql-host.png). Чтобы его узнать:
...
1. Проверка главной страницы сайта
   ...
   Для этого в браузере перейдите по адресу `http://<публичный-IP-адрес-ВМ>/`. После этого вы окажетесь на главной странице сайта и увидите ее глазами пользователя, посетившего ваш интернет-магазин. Предварительно не забудьте **Выйти** из панели администрирования вашим сайтом, в правом верхнем углу страницы. ля того, чтобы вернуться обратно в режим редактирования, необходимо авторизоваться на сайте с теми учетными данными, которые вы создавали для [администратора 1С-Битрикс.]( 

   ![Шаг 18](../_assets/bitrix-website/bitrix-website-ip-check.png)
